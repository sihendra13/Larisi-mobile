...
export default function CaptionScreen({
  ...
  const handleDoLaunch = async (overrideName, scheduledAt = null) => {
    ...
    try {
      ...
      if (scheduledAt) {
        showToast(`✓ ${toastLabel} berhasil dijadwalkan!`, 'success');
      } else {
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', height:'100dvh', overflow:'hidden', background: isGenZ ? '#0e0e12' : 'var(--m-bg)'}}>
          ...
          {(() => {
            ...
            return (
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {/* Button Jadwalkan (Icon Only) */}
                  <button
                    onClick={isOutOfQuota ? () => { if (triggerUpgrade) triggerUpgrade('Kuota Habis', 'Pilih paket untuk menambah kuota tayang iklan Anda:'); } : handleJadwalkanClick}
                  ...
                  <button
                    ...
                    disabled={posting || (!isOutOfQuota && !caption)}
                    style={{
                      padding:'10px 16px', height:'40px', borderRadius:'12px', flexShrink:0,
          ...
          <div style={{background:'#fff', borderRadius:'24px', padding:'40px 28px', textAlign:'center', maxWidth:'300px', width:'100%', boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            {launchPhase === 'loading' && (
              <>
                ...
                <div style={{fontFamily:'var(--m-font)', fontSize:'17px', fontWeight:'800', color:'var(--m-ink)', marginBottom:'6px'}}>
      ...
      {/* ── Schedule Post Bottom Sheet ── */}
      {showScheduleSheet && (
        <div style={{position:'fixed', inset:0, zIndex:2000, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'flex-end', justifyContent:'center'}}>
          <div style={{position:'relative', background:'#fff', borderRadius:'24px 24px 0 0', padding:'24px 20px', width:'100%', maxWidth:'480px', paddingBottom:'calc(24px + env(safe-area-inset-bottom))'}}>
            {/* Close button X */}
            <button
            ...
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--m-ink)" strokeWidth="2.5" strokeLinecap="round">
                <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
              </svg>
            </button>
            ...
            <div style={{fontFamily:'var(--m-font)', fontSize:'17px', fontWeight:'800', color:'var(--m-ink)', marginBottom:'4px'}}>Jadwalkan Tayang</div>
            <div style={{fontFamily:'var(--m-font)', fontSize:'13px', color:'var(--m-ink-sub)', marginBottom:'20px'}}>Pilih waktu postingan ini akan ditayangkan</div>
            ...
            <div style={{marginBottom:'16px'}}>
              ...
              <div style={{position:'relative'}}>
                ...
                <div style={{
                  ...
                  fontSize:'16px', 
                  fontWeight:'600', 
                  color: schedDate ? 'var(--m-ink)' : '#9CA3AF', 
                  background:'#F9F9FA', 
                  ...
                  {schedDate ? (function(){
                    ...
                    if(isNaN(d.getTime())) return schedDate;
                    const mName = ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Ags','Sep','Okt','Nov','Des'];
                    return `${d.getDate()} ${mName[d.getMonth()]} ${d.getFullYear()}`;
                  })() : 'Pilih Tanggal'}
                </div>
                {/* The Icon */}
            ...
            <button
              ...
              style={{width:'100%', padding:'15px', borderRadius:'14px', background:'#1A1A1A', color:'#fff', border:'none', cursor:'pointer', fontFamily:'var(--m-font)', fontSize:'15px', fontWeight:'800', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px'}}
            >
              Konfirmasi Jadwal
            </button>
          </div>
